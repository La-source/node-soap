...
WSDL.prototype.objectToXML = function(obj, name, nsPrefix, nsURI, isFirst, xmlnsAttr, schemaObject, nsContext) {
  ...
  if (Array.isArray(obj)) {
  ...
  } else if ( Array.isArray(obj[self.options.valueKey]) ) {
      var array = obj[self.options.valueKey];

      for (i = 0, n = array.length; i < n; i++) {
          parts.push(self.objectToXML(array[i], name, nsPrefix, nsURI, false, null, schemaObject, nsContext));
      }
    ...
    for (name in obj) {
      ...
      if (name === self.options.valueKey) {
        ...
        {
          return self.objectToXML(obj[name], name, nsPrefix, nsURI, false, null, schemaObject, nsContext);
        }
